...
@Service
public class BatchDeleteListener implements BatchProcessor {
    private final IBatchProductService batchService;
    private final JsonConverter converter;

    public BatchDeleteListener(IBatchProductService batchService, JsonConverter converter) {
        this.batchService = batchService;
        this.converter = converter;
    }

    @Override
    @SqsListener(queueNames = "product-delete-test")
    public CompletableFuture<Void> process(List<String> messages) {
        return converter.convertList(messages, Product.class)
                .flatMap(batchService::batchDelete)
                .toFuture();
    }
}
